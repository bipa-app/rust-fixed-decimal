...
impl<
        T: num_traits::ToPrimitive + ext_num_traits::ConstTenPow<E> + ops::Div<T, Output = T> + Copy,
        const E: u8,
    > num_traits::ToPrimitive for FixedDecimal<T, E>
{
    fn to_i64(&self) -> Option<i64> {
        (self.0 / T::RESULT).to_i64()
    }

    fn to_u64(&self) -> Option<u64> {
        (self.0 / T::RESULT).to_u64()
    }

    fn to_isize(&self) -> Option<isize> {
        (self.0 / T::RESULT).to_isize()
    }

    fn to_i8(&self) -> Option<i8> {
        (self.0 / T::RESULT).to_i8()
    }

    fn to_i16(&self) -> Option<i16> {
        (self.0 / T::RESULT).to_i16()
    }

    fn to_i32(&self) -> Option<i32> {
        (self.0 / T::RESULT).to_i32()
    }

    fn to_i128(&self) -> Option<i128> {
        (self.0 / T::RESULT).to_i128()
    }

    fn to_usize(&self) -> Option<usize> {
        (self.0 / T::RESULT).to_usize()
    }

    fn to_u8(&self) -> Option<u8> {
        (self.0 / T::RESULT).to_u8()
    }

    fn to_u16(&self) -> Option<u16> {
        (self.0 / T::RESULT).to_u16()
    }

    fn to_u32(&self) -> Option<u32> {
        (self.0 / T::RESULT).to_u32()
    }

    fn to_u128(&self) -> Option<u128> {
        (self.0 / T::RESULT).to_u128()
    }

    fn to_f32(&self) -> Option<f32> {
        let v = self.0.to_f32()?;
        let d = T::RESULT.to_f32()?;
        Some(v / d)
    }

    fn to_f64(&self) -> Option<f64> {
        let v = self.0.to_f64()?;
        let d = T::RESULT.to_f64()?;
        Some(v / d)
    }
}
